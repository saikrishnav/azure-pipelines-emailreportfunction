...
namespace EmailReportFunction.Utils
{
    ...
    {
        public static EmailReportConfiguration CreateConfiguration(string jsonRequest, ILogger logger)
        {
            var data = (JObject)JsonConvert.DeserializeObject(jsonRequest);
            var accessToken = data.GetJsonValue<string>("System.AccessToken");
