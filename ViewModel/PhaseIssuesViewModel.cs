...
namespace EmailReportFunction.ViewModel
{
    ...
    public class PhaseIssuesViewModel
    {
        ...
        public PhaseIssuesViewModel(List<PhaseData> phases)
        {
            Name = "Phase Issues";
            Tasks = new List<TaskResultViewModel>();
            foreach (var phase in phases)
            {
                if (phase != null && phase.Jobs != null)
                {
                    ...
                    if (canceledJobs.Any())
                    {
                        var failedJobsAsTasks = canceledJobs.Select(job => new TaskData() { Name = job.JobName, Issues = job.Issues, Status = job.JobStatus });
